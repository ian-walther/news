...
Workers should return normalized success, quality, and failure information. Failure kinds such as JavaScript required, auth required, paywall, blocking, or repeated insufficient content can trigger escalation. Network errors and timeouts should remain ordinary transient failures. Repeated simple-client rate limits without an explicit `Retry-After` may trigger a headless probe because publishers can use `429` for bot classification. Explicit retry timing remains authoritative for automatic scheduling, and a headless rate limit must not automatically escalate into the persistent headed browser.

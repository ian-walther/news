...
WORKER_DIR="${ROOT_DIR}/workers/extraction-headed-browser"
WORKERS_ROOT="${ROOT_DIR}/workers"

if [ "$#" -lt 1 ]; then
  echo "Usage: scripts/extract-headed-browser.sh URL [--summary|--raw|--text|--html]" >&2
  exit 64
fi
...
REQUEST="$(node -e 'process.stdout.write(JSON.stringify({schema_version: 1, implementation: "extraction.headed_browser", url: process.argv[1], options: {timeout_ms: 20000, minimum_text_length: 500}}))' "$URL")"
